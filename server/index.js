...
app.post("/transportistas", (req, res) => {
  ...
  db.query(
    "INSERT INTO transportistas(telefono,nombre,dot) VALUES (?,?,?)",
    [telefono, nombre, dot],
    (err, result) => {
      if (err) {
        console.log(err);
...
app.get("/autos", (req, res) => {
  db.query("SELECT marca,modelo,anio,fee FROM autos", (err, result) => {
    if (err) {
      console.log(err);
    } else {
      res.send(result);
...
app.put("/llegada", (req, res) => {
  ...
  db.query(
    "UPDATE pedidos SET fees=?,titulo=?,notas=? WHERE id=?",
    [fees, titulo, notas, id],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
...
app.put("/transportistas", (req, res) => {
  ...
  db.query(
    "UPDATE transportistas SET telefono=?,nombre=?,dot=? WHERE id=?",
    [telefono, nombre, dot, id],
    (err, result) => {
      if (err) {
        console.log(err);
      } else {
